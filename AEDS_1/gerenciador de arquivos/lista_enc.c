...
void armazenar(struct lista *cab,char nome[100], char tipo[5]){
	...
	if(strcmp(tipo,"txt")==0){
		...
		for(int i = 1; i <= 5; i++){
			...
			printf("p[%d] criado no endereco: %p\n",i,p);
...
void busca_interna(struct lista *cab){
	int block = 0;
	struct lista *p = cab->prox;
	struct lista *ant = p;
	printf("%s %s\n","Arquivo","Ocorrencias");
	if(p == NULL){
		...
		while(p != NULL){
			if(strcmp(arquivo,p->nome)==0){
				block += 1;
			}
			else if(strcmp(arquivo,p->nome)!=0){
				printf("%s %s\n",p->ant->nome,block);
				strcpy(arquivo,p->nome);
				printf("arquivo atual: %s\n",arquivo);
				//ant = p;
...
void apagar_arquivo(struct lista *p, struct lista *ant, char arquivo[105]){
	//printf("ok\n");
	//printf("ok\n");
	...
	while(p != NULL){
		...
		//printf("ant:%p\np:%p\n",ant,p);
		//printf("p->prox: %p\nnome: %s\n",p->prox,p->prox->nome);
		if(p != NULL && strcmp(p->nome,arquivo)==0){
			printf("entrou\n");
			if(p->prox != NULL){
				//printf("ok1\n");
				ant->prox = p->prox;
...
void remover_arquivo(struct lista *cab,char nome[105]){
	...
	if(p == NULL){
		...
		printf("arquivo nome: %s tipo: %s posicao  inicial: %p tamanho de blocos: %d\n",p->nome,p->tipo,p,p->total_block);
		char arquivo[105];
		strcpy(arquivo,p->nome);
		struct lista *ant = p->ant;
		printf("ant: %p e p: %p\n",ant,p);
		apagar_arquivo(p,ant,arquivo);
	}
	//apagar_arquivo(cab,nome);
}
